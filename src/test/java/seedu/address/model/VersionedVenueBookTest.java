...
public class VersionedVenueBookTest {

    private final ReadOnlySchedule addressBookWithAmy = new AddressBookBuilder().withPerson(MIDTERM).build();
    private final ReadOnlySchedule addressBookWithBob = new AddressBookBuilder().withPerson(WORKSHOP).build();
    private final ReadOnlySchedule addressBookWithCarl = new AddressBookBuilder().withPerson(CARL).build();
    private final ReadOnlySchedule emptyAddressBook = new AddressBookBuilder().build();

    @Test
    public void commit_singleAddressBook_noStatesRemovedCurrentStateSaved() {
        VersionedSchedule versionedAddressBook = prepareAddressBookList(emptyAddressBook);
    ...
    public void commit_multipleAddressBookPointerAtEndOfStateList_noStatesRemovedCurrentStateSaved() {
        VersionedSchedule versionedAddressBook = prepareAddressBookList(
    ...
    public void undo_singleAddressBook_throwsNoUndoableStateException() {
        ...
        assertThrows(VersionedSchedule.NoUndoableStateException.class, versionedAddressBook::undo);
    }
    ...
    public void redo_singleAddressBook_throwsNoRedoableStateException() {
        ...
        assertThrows(VersionedSchedule.NoRedoableStateException.class, versionedAddressBook::redo);
    }
    ...
    @Test
    public void equals() {
        VersionedSchedule versionedAddressBook = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);

        // same values -> returns true
        VersionedSchedule copy = prepareAddressBookList(addressBookWithAmy, addressBookWithBob);
        ...
        VersionedSchedule differentAddressBookList = prepareAddressBookList(addressBookWithBob, addressBookWithCarl);
        assertFalse(versionedAddressBook.equals(differentAddressBookList));

        // different current pointer index -> returns false
        VersionedSchedule differentCurrentStatePointer = prepareAddressBookList(
     ...
     */
    private void assertAddressBookListStatus(VersionedSchedule versionedAddressBook,
                                             List<ReadOnlySchedule> expectedStatesBeforePointer,
                                             ReadOnlySchedule expectedCurrentState,
                                             List<ReadOnlySchedule> expectedStatesAfterPointer) {
        // check state currently pointing at is correct
        assertEquals(new Schedule(versionedAddressBook), expectedCurrentState);
        ...
        for (ReadOnlySchedule expectedAddressBook : expectedStatesBeforePointer) {
            assertEquals(expectedAddressBook, new Schedule(versionedAddressBook));
            versionedAddressBook.redo();
        }

        // check states after pointer are correct
        for (ReadOnlySchedule expectedAddressBook : expectedStatesAfterPointer) {
            versionedAddressBook.redo();
            assertEquals(expectedAddressBook, new Schedule(versionedAddressBook));
        }
    ...
    /**
     * Creates and returns a {@code VersionedSchedule} with the {@code addressBookStates} added into it, and the
     * {@code VersionedSchedule#currentStatePointer} at the end of list.
     */
    private VersionedSchedule prepareAddressBookList(ReadOnlySchedule... addressBookStates) {
        assertFalse(addressBookStates.length == 0);

        VersionedSchedule versionedAddressBook = new VersionedSchedule(addressBookStates[0]);
     ...
     */
    private void shiftCurrentStatePointerLeftwards(VersionedSchedule versionedAddressBook, int count) {
